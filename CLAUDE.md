...
- **2026-04-22 (BUILD #6 — Admin-side reschedule + Owner booking alerts + QR codes)** — Three additions in one session:
  ...
  - **QR codes generated** — 3 variants saved under `assets/qr/` pointing at `https://shineproexterior.com`:
    - `shinepro-qr-plain.png` (780×780) — navy on white, simplest
    - `shinepro-qr-branded.png` (780×890) — logo in center + gold "SCAN TO BOOK" + URL label strip (main one to use)
    - `shinepro-qr-print-hires.png` (1640×1640) — print-ready for truck wrap / yard signs
    - Generated via Python `qrcode[pil]` library with `ERROR_CORRECT_H` (30% error correction) so the embedded logo doesn't break the scan.
  - **Files touched (BUILD #6):** `api/_emails.js` (added sendOwnerBookingAlert + estimateRevenue + sendOwnerMail + stripPhone helpers), `api/book.js` (Promise.all the two email sends), `api/reschedule.js` (Promise.all the two email sends), `api/admin-reschedule.js` (new), `admin.html` (reschedule modal + button on job cards + wireReschedDelegation + openReschedModal + loadReschedSlots + showToast). Plus 3 new QR assets in `assets/qr/`. All CRLF-applied.
  - **No new env vars required.** Reuses the existing Resend setup.
